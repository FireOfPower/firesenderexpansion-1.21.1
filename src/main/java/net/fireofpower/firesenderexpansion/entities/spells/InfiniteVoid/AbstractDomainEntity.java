...
public abstract class AbstractDomainEntity extends Entity implements AntiMagicSusceptible, INBTSerializable<CompoundTag> {
                ...
                .forEach(e -> {
                            if(e.distanceTo(this) < getRadius() && !Objects.equals(e,this)){
                                ...
                                if(!e.getClashable()){
                                    ...
                                    FiresEnderExpansion.LOGGER.debug("REFINEMENT DIFFERENCE TOO GREAT (Opponent) - NO CLASH");
                                    destroyDomain();
                                }else if ((double) getRefinement() / e.getRefinement() >= AcesSpellUtilsConfig.refinementDifference){
                                    FiresEnderExpansion.LOGGER.debug("REFINEMENT DIFFERENCE TOO GREAT (Self) - NO CLASH");
                                    e.destroyDomain();
                                }else{
                                    ...
                                    if(getClashingWith() != null && e.getClashingWith() != null) {
                                        ...
                                        FiresEnderExpansion.LOGGER.debug("Marking domains as clashing");
                                        setClashing(true);
                                        e.setClashing(true);
                                    }
                                }
    ...
    public void destroyDomain(){
        FiresEnderExpansion.LOGGER.debug("Domain cleared O.o (ps this is from client:{})",level().isClientSide());
        discard();
    ...
    public void tick() {
        ...
        if(getOwner() instanceof LivingEntity living && living.isDeadOrDying()){
            ...
            FiresEnderExpansion.LOGGER.debug("Owner is dead");
            destroyDomain();
        }
        if(!level().isClientSide && getClashingWith().isEmpty() && isClashing()){
            //If you're not actually clashing but you think you are, then update the isClashing() return value
            FiresEnderExpansion.LOGGER.debug("Hello from client:" + level().isClientSide + " isClashing mistakenly true");
            setClashing(false);
        }
        if(isClashing()) {
